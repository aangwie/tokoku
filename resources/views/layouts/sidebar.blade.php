    ...
    <div class="flex-grow overflow-y-auto px-4 py-6 space-y-1.5 scrollbar-none">
        ...
        @else
            ...
            <a href="{{ route('orders.index') }}" class="flex items-center py-2.5 rounded-xl text-sm font-medium transition duration-150 {{ request()->routeIs('orders.*') ? 'bg-[#91ebff]/10 text-white font-bold border-l-2 border-[#91ebff] pl-3.5 pr-4' : 'text-slate-400 hover:bg-slate-800/60 hover:text-white px-4' }}" :class="sidebarMinimized ? 'justify-center' : 'gap-3'">
                ...
                <span x-show="!sidebarMinimized" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">Riwayat Pesanan</span>
            </a>

            <!-- Daftar Pesanan -->
            <a href="{{ route('orders.index') }}" class="flex items-center py-2.5 rounded-xl text-sm font-medium transition duration-150 {{ request()->routeIs('orders.index') && !request()->routeIs('orders.show') ? 'bg-[#91ebff]/10 text-white font-bold border-l-2 border-[#91ebff] pl-3.5 pr-4' : 'text-slate-400 hover:bg-slate-800/60 hover:text-white px-4' }}" :class="sidebarMinimized ? 'justify-center' : 'gap-3'">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
                <span x-show="!sidebarMinimized" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">Daftar Pesanan</span>
            </a>
        @endif
    ...
    <div x-show="sidebarOpen" 
        ...
        <div class="flex-grow overflow-y-auto px-4 py-6 space-y-1.5 scrollbar-none">
            ...
            @else
                ...
                <!-- Daftar Pesanan -->
                <a href="{{ route('orders.index') }}" class="flex items-center gap-3 py-2.5 rounded-xl text-sm font-medium transition duration-150 {{ request()->routeIs('orders.index') && !request()->routeIs('orders.show') ? 'bg-[#91ebff]/10 text-white font-bold border-l-2 border-[#91ebff] pl-3.5 pr-4' : 'text-slate-400 hover:bg-slate-800/60 hover:text-white px-4' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                    </svg>
                    Daftar Pesanan
